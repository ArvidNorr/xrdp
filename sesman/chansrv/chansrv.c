...
channel_thread_loop(void* in_val)
{
  ...
  if (error == 0)
  {
    ...
    while (g_obj_wait(objs, num_objs, 0, 0, timeout) == 0)
    {
      ...
      {
        log_message(LOG_LEVEL_ERROR,"channel_thread_loop: g_term_event set");
        clipboard_deinit();
        sound_deinit();
        dev_redir_deinit();
        break;
      ...
      if (g_lis_trans != 0)
      {
        ...
        {
          log_message(LOG_LEVEL_ERROR,"channel_thread_loop: trans_check_wait_objs error");
        }
      }
      if (g_con_trans != 0)
      {
        ...
        {
          log_message(LOG_LEVEL_ERROR,"channel_thread_loop: "
