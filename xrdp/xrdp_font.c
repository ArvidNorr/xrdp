...
xrdp_font_create(struct xrdp_wm* wm)
{
  ...
  if (!g_file_exist(file_path))
  {
    log_message(LOG_LEVEL_ERROR,
            "xrdp_font_create: error font file [%s] does not exist",file_path);
    return 0;
  }
  ...
  {
    log_message(LOG_LEVEL_ERROR,
            "xrdp_font_create: error reading font from file [%s]",file_path);
    return 0;
  }
    ...
    if (b > 0)
    {
      ...
      while (s_check_rem(s, 16))
      {
        ...
        if (datasize < 0 || datasize > 512)
        {
          /* shouldn't happen */
          log_message(LOG_LEVEL_ERROR,
                  "error in xrdp_font_create, datasize wrong");
          log_message(LOG_LEVEL_ERROR,
